 ...
 * 加载网络图片
 */
class ImageUrlActivity : MVActivity<ActivityUrlBinding, ImageUriVM>() {
...
//                }
    ...
    fun onRecyclerScrollStateChanged(recyclerView: RecyclerView?, newState: Int) {
        if (newState == RecyclerView.SCROLL_STATE_IDLE) setImageLoader(firstVisibleItem, lastVisibleItem, binding.recycler)
        else cancelAlltasks()
    }
